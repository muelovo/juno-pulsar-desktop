...
Windows 10/11 x64 的非官方桌面飞行物应用。Tauri 2 + React + TypeScript + Rust/Win32，包含原创生成皮肤、程序化特效与本地合成音效。

**当前是 0.4.0 开发预览，不是已经完成真实桌面删除验收的稳定版。** 删除默认关闭；启用后在桌面项目上松开会直接移入回收站。桌面挂载与混合 DPI 兼容性请按手工清单验证。
...
3. 开启回收站操作后，拖到桌面实际文件／快捷方式／普通文件夹并松开，目标会直接移入回收站。
4. Esc 或右键可在松手前取消。所有目标仍由 Shell PIDL 命中并经过受保护路径策略校验。
...
- 文件身份在命中和 Shell 执行前重验，但 IFileOperation 是路径／Shell 项目操作，不能宣称消除恶意同用户进程在最后一瞬间替换路径的全部竞态。本版不适用于存在主动本机攻击者的环境。
